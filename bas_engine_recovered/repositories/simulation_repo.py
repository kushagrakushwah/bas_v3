        ...
        async with AsyncSessionLocal() as session:
            ...
            for mod in module_results:
                ...
                for finding in mod.findings:
                    # #58 fix: use upsert instead of a bare INSERT so that a
                    # caller-supplied duplicate UUID does not crash the worker.
                    # ON CONFLICT (id) DO UPDATE means duplicate findings are
                    # silently updated rather than raising IntegrityError.
